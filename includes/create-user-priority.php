		...
		// get variables from the URL/form and sql injection prevention
			...
			//Loop through array in order to find role id
 			foreach ($prtyid as $prty) {
 				if ($stmt = $user_role->runQuery("INSERT INTO user_priority_list (usrpr_id , user_id, created_at, updated_at) VALUES (?, ?, now(),now())"))
				{
					$stmt->bind_param("ii", $prty, $id);
					$stmt->execute();
					$stmt->close();
					$msg = "<div class='alert alert-success'>
             		<span class='glyphicon glyphicon-info-sign'></span> &nbsp; Priority Successfully Added to User
           		 	</div>";
				}
