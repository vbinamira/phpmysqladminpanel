...
if (isset($_GET['id']))
{
	...
	// make sure the 'id' in the URL is valid
		...
		// get variables from the URL/form and sql injection prevention
			...
			//Loop through array in order to find role id
 			foreach ($roleid as $role) {
 				if ($stmt = $user_role->runQuery("UPDATE role_user SET role_id = ?, updated_at=now() WHERE user_id = ?"))
				{
					$stmt->bind_param("ii", $role, $id);
					$stmt->execute();
					$stmt->close();
					$msg = "<div class='alert alert-success'>
             		<span class='glyphicon glyphicon-info-sign'></span> &nbsp; User Role Successfully Changed
           		 	</div>";
				}
		...
		{
			$msg = "<div class='alert alert-danger'>
             		<span class='glyphicon glyphicon-info-sign'></span> &nbsp; An error occured! Please contact IT immediately!
           		 	</div>";
		}
	}
